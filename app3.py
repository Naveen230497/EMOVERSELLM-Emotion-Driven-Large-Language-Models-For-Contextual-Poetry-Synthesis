...
**STRICT REQUIREMENTS:**
1. Each stanza MUST contain exactly 4 lines
2. Use 5-7 of these signature phrases: {super_emphasized_phrases}
3. Maintain average line length: {author_style['avg_sentence_length']:.1f} words (±0.5)
4. Focus on: {emotion.lower()} (use related words 8+ times)

**POETIC TECHNIQUES:**
- Perfect ABAB rhyme scheme
- 5+ literary devices
- Flawless meter
- Vivid imagery
- Rich vocabulary
...
[Stanza 4 - 4 lines]"""
        ...
        response = client.chat.completions.create(
            ...
            temperature=min(0.3 + (intensity * 0.01), 0.5),
            max_tokens=1500,
            top_p=0.8,
        ...
        # Clean up any unwanted references
        poem = re.sub(r'❚❚\[\[.*?\]\]❚❚', '', poem)
        poem = re.sub(r'www\.poemhunter\.com.*?(?:\s|$)', '', poem, flags=re.IGNORECASE)
        ...
        for line in lines:
            ...
            if len(current_stanza) == 4:  # When we have 4 lines
                stanzas.append(current_stanza)
                current_stanza = []
        
        # Rebuild poem with proper 4-line stanzas
        formatted_poem = "\n\n".join(["\n".join(stanza) for stanza in stanzas])
        
        return formatted_poem, validate_poetic_structure(formatted_poem)
    except:
        return None, 0.98
